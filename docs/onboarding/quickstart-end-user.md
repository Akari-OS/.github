...
- **左 (Source)** — Pool / Work 一覧、素材パネル、Style など、入力リソース
- **中央 (Editor / Preview)** — 編集領域（Writer ならエディタ、Video ならタイムライン + プレビュー）
- **右 (Partner + Inspector)** — AI チャットがメイン、必要に応じてインスペクター（選択中オブジェクトの詳細）に切替

> **左から右へ流れる動線**: 「あなたの素材」を起点に、「AI と一緒に仕上げる」自然な流れ。
> Partner (AI) は常に手元 = 右側にあり、いつでも呼び出せます。
